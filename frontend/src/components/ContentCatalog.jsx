...
// Danh mục nội dung đào tạo (thêm/sửa/xóa + lọc). Dùng ở màn Danh mục và tab Nội dung trong Chương trình.
  ...
  return (
    <>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', margin: '4px 0 8px' }}>
        ...
        <span style={{ flex: 1 }} />
        {canManage && <button className="btn-sm" onClick={() => setForm({ ...EMPTY, order: rows.length + 1 })}>+ Thêm nội dung</button>}
      </div>
      ...
      {loading ? <p className="muted-note">Đang tải...</p> : (
        <Table>
          ...
          <tbody>
            ...
            {shown.length === 0 && <tr><td colSpan={canManage ? 7 : 6} className="muted-note">Chưa có nội dung nào. (Nạp Ma_Khoa_Hoc từ màn Nhân sự → "Nạp lịch sử".)</td></tr>}
          </tbody>
        </Table>
      )}
